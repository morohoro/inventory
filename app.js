...
const backgroundImageUrl = '.\/images\/90s-background.jpg';
...
function handleDragDrop() {
    $(".item-drag").draggable({
        ...
        start: function(event, ui) {
            ...
            if (!itemData.useable) {
                $("#item-use").css("background", "rgba(37, 39, 41, 0.8");
            }
        ...
        stop: function() {
            ...
            $(this).css("background", "rgba(37, 39, 41, 0.8)");
            $(this).find("img").css("filter", "brightness(100%)");
            $("#item-use").css("background", "rgba(" + InventoryOption + ", 0.8)");
        },
    });
            ...
            if (fromData.amount == $toAmount) {
                ...
                if (newData.name.split("_")[0] == "weapon") {
                    if (!Inventory.IsWeaponBlocked(newData.name)) {
                        ...
                        var QualityColor = "rgba(255, 0, 57, 0.8)";
                        // if (newData.info.quality < 25) {
                        //     QualityColor = "rgba(192, 57, 43, 1)";
                        // } else if (newData.info.quality > 25 && newData.info.quality < 50) {
                        //     QualityColor = "rgba(230, 126, 34, 1)";
                        // } else if (newData.info.quality >= 50) {
                        //     QualityColor = "rgba(255, 0, 57, 0.8)";
                        // }
                        if (newData.info.quality !== undefined) {
...
function InventoryError($elinv, $elslot) {
    $elinv
        ...
        .css("background", "rgba(255, 0, 57, 0.8)")
        .css("transition", "background 500ms");
    setTimeout(function() {
        $elinv
            .find("[data-slot=" + $elslot + "]")
            .css("background", "rgba(37, 39, 41, 0.8)");
    }, 500);
...
(() => {
    ...
    Inventory.Open = function(data) {
        ...
        $("#player-inv-label").html(data.citizenid);
